...
const TopViewedArticles = () => {
    ...
    return (
        <div className='bg-[#060606] mb-3'>
            <div className='container mx-auto p-6 lg:p-10 bg-white/4 rounded-3xl'>
                <figure>
                    <Image src={icBook} alt='' />
                </figure>
                <p className='text-white text-xl lg:text-2xl font-semibold mt-4 mb-6 lg:mt-5 lg:mb-8'>{t("topArticleTitle")}</p>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-4'>
                    {topArticles.map((item) => {
                        ...
                        return (
                            <div
                                onClick={() => handleOnclickSetFAQ(item.id)}
                                key={item.id}
                                className={`${isActive && "bg-white/4 py-4"} rounded-lg`}>
                                <div className={`group flex items-center justify-between space-x-4 py-3 cursor-pointer rounded-lg transition-all ease-linear duration-100 ${!isActive ? "hover:bg-white/4 hover:p-4" : "p-4"}`}>
                                ...
                                {isActive && <div className='text-[#848484] text-sm font-normal py-4 px-4 lg:px-6 flex flex-col gap-y-4'>
                                    {item.answer.split('\n').map((item, index) => {
