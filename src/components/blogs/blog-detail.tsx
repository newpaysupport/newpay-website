...
import { blogSections, blog as enBlog } from '@/i18n/messages/en.json';
...
const BlogDetail = () => {
    ...
    const t = useTranslations('blogSections');
    ...
    return (
        <div className="container mx-auto px-4 py-6">
                ...
                <div className="w-full lg:w-[80%]">
                    ...
                    {blogSections.map((section, index) => (
                        <div key={index} className="mb-12">
                            <h2 className="font-semibold text-lg md:text-xl mb-4">{t(`${index}.title`)}</h2>
                            <div className="text-sm md:text-base flex flex-col gap-4 text-justify">
                                {section.paragraphs?.map((text, idx) => (
                                    <p key={idx}>{t(`${index}.paragraphs.${idx}`)}</p>
                                ))}
                                {section.bullets && (
                                    <ul className="list-disc pl-5 flex flex-col gap-3">
                                        {section.bullets.map((item, i) => (
                                            <li key={i}>{t(`${index}.bullets.${i}`)}</li>
                                        ))}
                                    </ul>
                                )}
                                {section.paragraphsAfterBullets?.map((text, idx) => (
                                    <p key={`after-${idx}`}>{t(`${index}.paragraphsAfterBullets.${idx}`)}</p>
                                ))}
                            </div>
                        </div>
                    ))}
            ...
            {/* Related Posts */}
            <div className="mt-16">
                <h2 className="text-2xl md:text-3xl font-semibold mb-6">{locale == 'en' ? "Further Reading" : '进一步阅读'}</h2>
